...
async fn run_stream(sse_body: &str) -> Vec<ResponseEvent> {
    run_stream_with_mode(sse_body.as_bytes(), true).await
}

async fn run_stream_with_bytes(sse_body: &[u8]) -> Vec<ResponseEvent> {
    run_stream_with_mode(sse_body, true).await
}

async fn run_stream_aggregated_only(sse_body: &str) -> Vec<ResponseEvent> {
    run_stream_with_mode(sse_body.as_bytes(), false).await
}

async fn run_stream_with_mode(
    sse_body: &[u8],
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn assistant_text_precedes_tool_call() {
    if network_disabled() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Codex sandbox."
        );
    ...
    let sse = concat!(
        "data: {\"choices\":[{\"delta\":{\"content\":\"preamble\"}}]}\n\n",
        "data: {\"choices\":[{\"delta\":{\"tool_calls\":[{\"id\":\"call_1\",\"type\":\"function\",\"function\":{\"name\":\"run\",\"arguments\":\"{}\"}}]},\"finish_reason\":\"tool_calls\"}]}\n\n",
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn reasoning_and_text_precede_tool_call() {
    if network_disabled() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Codex sandbox."
        );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn aggregated_assistant_text_precedes_tool_call() {
    if network_disabled() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Codex sandbox."
        );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn aggregated_reasoning_precedes_tool_call() {
    if network_disabled() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Codex sandbox."
        );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn aggregated_reasoning_and_text_precede_tool_call() {
    if network_disabled() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Codex sandbox."
        );
